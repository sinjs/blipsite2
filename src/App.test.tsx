...
test("are the mod commands rendering", () => {
  render(<ModCommands />);
  const MCommandsElem = screen.queryByText(
    /kicks the mentioned user from the server/i
  );
...
test("are the fun commands rendering", () => {
  render(<FunCommands />);
  const FCommandsElem = screen.queryByText(
    /displays a random meme from reddit/i
  );
  expect(FCommandsElem).not.toBeNull();
});

test("are the music commands rendering", () => {
  render(<MusicCommands />);
