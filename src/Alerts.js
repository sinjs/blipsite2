...
class Alerts extends React.Component {
    ...
    async componentDidMount() {
        const response = await axios.get('https://sinmineryt.ga/blip-status.json', {responseType: 'json'});
        const AlertExists = response.data.exists;
        const AlertHeading = response.data.heading;
        const AlertText = response.data.text;

        await this.setStateAsync({alertExists: AlertExists, alertHeading: AlertHeading, alertText: AlertText})
    }
